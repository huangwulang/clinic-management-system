...
export function hashPassword(password: string) {
  return MD5_HEX_PATTERN.test(password) ? password.toLowerCase() : md5(password);
}

export function hashPasswordFields<T extends Record<string, any>>(data: T, fields: string[]): T {
  const payload = { ...data };
  fields.forEach((field) => {
    if (typeof payload[field] === "string" && payload[field]) {
      payload[field] = hashPassword(payload[field]);
    }
  });
  return payload;
}
