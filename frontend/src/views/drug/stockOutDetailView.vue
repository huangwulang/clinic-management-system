    ...
    <section v-loading="loading" class="detail-card">
      <header class="detail-header">
        ...
        <div class="header-actions">
          ...
          <button v-if="showEdit" class="action-btn outline" type="button" @click="editStockOut">
      ...
      <section class="info-panel">
        <div><span>出库订单：</span><strong>{{ displayValue(order?.orderNo) }}</strong></div>
        <div><span>出库日期：</span><strong>{{ stockDate }}</strong></div>
        <div><span>出库人员：</span><strong>{{ displayValue(order?.operator) }}</strong></div>
        <div><span>出库类型：</span><strong>{{ stockTypeLabel }}</strong></div>
        <div><span>制单日期：</span><strong>{{ documentDate }}</strong></div>
        <div><span>制单人员：</span><strong>{{ displayValue(order?.maker) }}</strong></div>
        <div class="remark"><span>出库备注：</span><strong>{{ remark }}</strong></div>
...
import { stockOutOrderApi } from "@/api";
...
const title = computed(() => `出库详情-${status.value}`);
const remark = computed(() =>
  remarkData.value.remark
    || (isRejected.value ? "出库信息审核未通过，请修改后重新提交。" : "无"),
);
...
onMounted(async () => {
  if (!orderId) {
    ElMessage.error("缺少出库单 ID，无法加载详情");
    return;
  }
  loading.value = true;
  try {
    const response: any = await stockOutOrderApi.get(orderId);
    order.value = response?.data || null;
    if (!order.value) {
      ElMessage.error("未查询到出库单数据");
    }
  } catch {
    ElMessage.error("出库详情加载失败，请稍后重试");
  } finally {
    loading.value = false;
  }
});
...
const auditOrder = async () => {
  if (!orderId) return;
  await stockOutOrderApi.audit(orderId, { approved: true, remark: "审核通过" });
...
const deleteOrder = async () => {
  if (!orderId) return;
  await stockOutOrderApi.remove(orderId);
