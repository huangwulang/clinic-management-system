        ...
        <table class="medicine-table">
          ...
          <tbody>
            ...
            <tr v-if="selectedMedicines.length === 0">
              <td class="medicine-empty" colspan="13">请点击“添加药品”选择出库药品</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const stockTypeOptions = [
  { label: "科室领药", value: "DEPARTMENT_OUT" },
  { label: "报损出库", value: "DAMAGE_OUT" },
  { label: "采购退货", value: "PURCHASE_RETURN_OUT" },
];
const generateStockOutOrderNo = () =>
...
const formData = reactive({
  orderNo: generateStockOutOrderNo(),
  stockDate: "",
  documentDate: "",
  operatorId: "",
  stockType: "DEPARTMENT_OUT",
  maker: "",
  remark: "",
});

const pageTitle = computed(() =>
  isEditMode ? "编辑出库信息" : isRestockMode ? "再次出库" : "新增出库信息",
);
...
const goBack = () => {
  router.push("/drug/stock-out");
};


onMounted(async () => {
  const [employeeResponse, userResponse, orderResponse]: any[] = await Promise.all([
    staffApi.page({ page: 1, size: 200 }),
    authApi.me(),
    sourceOrderId ? stockOutOrderApi.get(sourceOrderId) : Promise.resolve(null),
  ]);
  ...
  const stockTypeValueMap: Record<string, string> = {
    ...
    采购出库: "PURCHASE_RETURN_OUT",
  };
  formData.stockType = stockTypeValueMap[String(order.stockType || "")]
    || "DEPARTMENT_OUT";
  if (isEditMode) {
...
const saveOrder = async (direct: boolean) => {
  const payload = buildOrderPayload();
  if (!payload) return;
  const response: any = isEditMode && sourceOrderId
    ? await stockOutOrderApi.update(sourceOrderId, payload)
    : await stockOutOrderApi.create(payload);
  const id = response?.data?.id || (isEditMode ? sourceOrderId : "");
  if (id) {
    if (direct) {
      await stockOutOrderApi.approve(String(id));
    } else {
      await stockOutOrderApi.submit(String(id));
    }
  }
  ElMessage.success(direct ? "单据已保存并完成出库" : "单据已保存并提交审核");
  router.push("/drugs/stock-out");
};
...
const handleAudit = async (approved: boolean) => {
  ...
  try {
    await stockOutOrderApi.update(sourceOrderId, payload);
    const response: any = await stockOutOrderApi.audit(sourceOrderId, { approved });
